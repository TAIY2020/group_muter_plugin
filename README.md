...
**权限控制**：默认任何群成员都可以通过对话让麦麦自行判断是否主动沉默（符合“可被礼貌请求安静”的设计）。如果只想让管理员能这样诱导，把 `tool.require_admin` 设为 `true`——开启后，与非管理员对话时（或解析不出发起用户时）即使模型想调用 `silence` 也会被拒绝。注意这与关键词静音的管理员校验是各自独立的开关。
...
| `tool.require_admin` | bool | `false` | 是否仅允许管理员所在的对话诱导主动沉默（默认关闭，任何人可礼貌请求麦麦安静） |
