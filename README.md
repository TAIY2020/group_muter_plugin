...
> 本版本 (v2.0.0) 基于 **MaiBot SDK v2** 重写，使用 `@HookHandler` 装饰器实现入站/出站双重拦截，配合 `PluginConfigBase` 强类型配置模型，支持配置热重载和 Dashboard UI 配置。
...
- **高度可配置**: 静音时长、触发关键词均可在配置文件或 Dashboard 中轻松修改。
...
下载 `group_muter_plugin` 文件夹放入麦麦主程序的 `plugins` 目录下，然后重启主程序即可完成插件的注册和加载。
...
首次启动麦麦后，插件会在其目录下自动生成 `config.toml` 文件。配置管理员权限后重启麦麦主程序即可。你也可以通过 **Dashboard** 在线修改配置，修改后会自动热重载生效。
...
- **`user_control.list`**: 这是一个**核心安全设置**。
  ...
  - **您必须手动编辑此文件**（或通过 Dashboard），将管理员的 QQ 号（字符串格式）添加到这个列表中，才能使用本插件的命令。例如：`list = ["12345", "67890"]`。
